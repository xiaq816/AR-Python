...
def make_look_at_matrix(
    ...
    center_local = transform3(orig_world_to_eye, center) # 世界->相机
    z_dir_local = center_local / np.linalg.norm(center_local) # 相机坐标标准化
    delta_r_local = from_two_vectors(
        np.array([0, 0, 1], dtype=center.dtype), z_dir_local
    ) # 相机系中z轴与center之间的旋转矩阵
    orig_eye_to_world = np.linalg.inv(orig_world_to_eye) # 变换矩阵 相机->世界

    new_eye_to_world = orig_eye_to_world.copy()
    new_eye_to_world[0:3, 0:3] = orig_eye_to_world[0:3, 0:3] @ delta_r_local # 将相机坐标转到以center点为中心  Tcw_new = Tcw * dR^(-1)
