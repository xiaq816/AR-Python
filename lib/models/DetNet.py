...
class DetNet(nn.Module):
    def __init__(self):
        ...
        self.center_network = self.make_center_network()
        self.length_network = self.make_length_network()
    ...
    def make_length_network(self):
        out = nn.Sequential(
            nn.Conv2d(160, 2, kernel_size=1, stride=1),
            nn.BatchNorm2d(2),
            nn.ReLU6(inplace = True),
            nn.AdaptiveAvgPool2d(1),
        )
    ...
    def forward(self, x):
        x = self.backbone(x)
        center = self.center_network(x)
        center = center.view(-1, 2, 2)
        length = self.length_network(x)
        cls = self.cls_network(x)

        center_length = torch.cat((center, length.mean(-1).mean(-1).unsqueeze(-1)), 2)
        cls = cls.mean(-1).mean(-1)

        return center_length, cls
