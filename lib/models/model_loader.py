...
from .stgcn_SH22 import Model as stgcn_SH  #stgcn_tiny_SH17
from .stgcn_tiny_fullgraph_HH4 import Model as stgcn_HH
...
def load_detection_model(model_path: str):
    ...
    det_model.load_state_dict(weights)

    # det_model = det_model.cuda()

    return det_model

def load_gesture_model(model_path: list):
    SH_ges_rec_model = stgcn_SH(num_class=22, num_point=21, num_person=1, graph='lib.models.graph.Gesture.Graph',
                                graph_args={'labeling_mode':'spatial'}, in_channels=3)

    # HH_ges_rec_model = stgcn_HH(num_class=4, num_point=21, num_person=2, graph='lib.models.graph.Gesture.Graph',
    #                             graph_args={'labeling_mode':'spatial'}, in_channels=3)
    HH_ges_rec_model = stgcn_HH(num_class=4, num_point=42, num_person=1, graph='lib.models.graph.Gesture.Graph',
                                graph_args={'labeling_mode':'spatial'}, in_channels=3)
    weights_SH = torch.load(model_path[0])
    weights_HH = torch.load(model_path[1])
    SH_ges_rec_model.load_state_dict(weights_SH)
    HH_ges_rec_model.load_state_dict(weights_HH)

    return SH_ges_rec_model, HH_ges_rec_model